...
def set_chosen_single(driver, id, value):
    ...
    for result in results:
        ...
        if result.text == value:
            ...
            return
    raise ValueError("Could not find value: '" + str(
        value) + "' for chosen-single-select: '" + id + "'. Values availible: " + ', '.join(values))
...
def set_chosen_multi(driver, id, values):
    ...
    for value in values:
        ...
        if not found:
            raise ValueError("Could not find value: '" + str(
                value) + "' for chosen-multi-select: '" + id + "'. Values availible: " + ' '.join(values))
...
def set_date(driver, id, date):
    ...
    date_picker = WebDriverWait(driver, 3).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, 'div.bootstrap-datetimepicker-widget')))
    ...
    if 'date' in action:
        # open date picker
        if 'time' in action:
            date_picker_parent = WebDriverWait(date_picker, 3).until(
                EC.presence_of_element_located((By.CSS_SELECTOR, 'li.collapse div.datepicker'))).find_element_by_xpath(
                '..')
        ...
        months = WebDriverWait(date_picker, 3).until(
            EC.presence_of_all_elements_located(
                (By.CSS_SELECTOR, 'div.datepicker-months table tbody tr td span.month')))
        for month in months:
            if month.get_attribute('innerHTML') == date.strftime('%b'):
                if 'disabled' in month.get_attribute('class'):
                    raise ValueError("Rounded date: " + date.strftime('%Y-%m-%d %H:%M') + " from: " + date.strftime(
                        '%Y-%m-%d %H:%M') + " has month out of valid range.")
                else:
                    month.click()
                    break
        ...
        for day in days:
            if int(day.get_attribute('innerHTML')) == date.day and 'old' not in day.get_attribute(
                    'class') and 'new' not in day.get_attribute('class'):
                if 'disabled' in day.get_attribute('class'):
                    raise ValueError("Rounded date: " + date.strftime('%Y-%m-%d %H:%M') + " from: " + date.strftime(
                        '%Y-%m-%d %H:%M') + " has day out of valid range.")
                else:
                    day.click()
                    break
...
def get_chosen_multi(driver, id):
    ...
    id += "_chosen"
    chosen = WebDriverWait(driver, 3).until(
        EC.element_to_be_clickable((By.XPATH, './/div[@id="' + id + '"]/ul[@class="chosen-choices"]')))
        ...
        if 'phantomjs' in driver_environment_variable:
            ...
            if platform == "linux" or platform == "linux2":
                ...
                return webdriver.Chrome(chrome_options=chrome_options,
                                        executable_path=BASE_DIR + '/testing/selenium_drivers/chromedriver')
            else:
            ...
            'phantomjs', 'chrome', 'chrome-headless', 'raspberrypi', driver_environment_variable))
    else:
...
def assert_exists(client, id, exists=True):
    ...
    if exists:
        ...
        try:
            ...
            assert False, "Exception %r occured while searching for Element %r. Expected one of: TimeoutException, ElementNotVisibleException, NoSuchElementException" % (
            e, id)
